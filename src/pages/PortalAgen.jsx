...
export default function PortalAgen() {
  ...
  const load = useCallback(async () => {
    ...
    const [pendaftaranRes, komisiRes, profilRes, leadsRes, paketRes, tiketRes] = await Promise.all([
      ...
      supabase.from('tiket_bantuan').select('*').order('updated_at', { ascending: false }),
    ]);
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="font-display font-semibold">Bantuan</h2>
        <button type="button" onClick={openAddTiket} className="bg-accent-soft hover:bg-accent-soft-hover text-accent-text font-semibold py-1.5 px-3 rounded-md2 text-xs">
          + Buat Tiket
        </button>
      </div>
      <div className="card rounded-xl2 overflow-hidden mb-8">
